'use strict';

class HashMap {
    constructor(initialCapacity=8) {
        this.length = 0;
        this._slots = [];
        this._capacity = initialCapacity;
        this._deleted = 0;
    }

    get(key) {
        const index = this._findSlot(key);
        if (this._slots[index] === undefined) {
            throw new Error('Key error');
        }
        return this._slots[index].value;
    }

    set(key, value) {
        const loadRatio = (this.length + this._deleted + 1) / this._capacity;
        if (loadRatio > HashMap.MAX_LOAD_RATIO) {
            this._resize(this._capacity * HashMap.SIZE_RATIO);
        }

        const index = this._findSlot(key);
        if(this._slots[index]) {
            this.length--;
        }
        this._slots[index] = {
            key,
            value,
            deleted: false
        };
        this.length++;
    }

    remove(key) {
        const index = this._findSlot(key);
        const slot = this._slots[index];
        if (slot === undefined) {
            throw new Error('Key error');
        }
        slot.deleted = true;
        this.length--;
        this._deleted++;
    }

    _findSlot(key) {
        const hash = HashMap._hashString(key);
        const start = hash % this._capacity;

        for (let i=start; i<start + this._capacity; i++) {
            const index = i % this._capacity;
            const slot = this._slots[index];
            if (slot === undefined || (slot.key == key && !slot.deleted)) {
                return index;
            }
        }
    }

    _resize(size) {
        const oldSlots = this._slots;
        this._capacity = size;
        // Reset the length - it will get rebuilt as you add the items back
        this.length = 0;
        this._deleted = 0;
        this._slots = [];

        for (const slot of oldSlots) {
            if (slot !== undefined && !slot.deleted) {
                this.set(slot.key, slot.value);
            }
        }
    }

    static _hashString(string) {
        let hash = 5381;
        for (let i=0; i<string.length; i++) {
            hash = (hash << 5) + hash + string.charCodeAt(i);
            hash = hash & hash;
        }
        return hash >>> 0;
    }
}
function is_palindrome(string) {
  
    //create new hash map
    let newHashMAp = new HashMap();
    let counter = 0;
    //loop through 
    //increase counter
    for(let i = 0; i < string.length; i++){
        newHashMAp[string.charAt(i)]++;      
    }

    //check each value
    for(let j = 0; j< newHashMAp.length && count <=1; j++){
        counter += newHashMAp[key] % 2;
        
    }
    return counter <=1
}
//count 
//
// Step 1. Create a Hashmap where key is character and value is count(number of times character occur).
// Step 2. Iterate string 1, populate the hashmap and increment the count for repeated characters.
// Step 3. Iterate string 2, for each character, check character is present in map, if not, then stop as it not Anagram. If character is present then decrement count. 
// Also, remove element from hashmap if count is 0.
// Step 4. At last, If hashmap is empty, then strings are anagram otherwise not.
function anagram(arr){
    let hash = new HashMap();
    for(let i=0; i < arr.length; i++){
        let key = arr[i].split('')
            .sort()
            .join('');
        try{
            let val = hash.get(key);
            val.push(arr[i]);
            }
            catch(err){
                hash.set(key, [arr[i]])     
            }
    }
    let arrayResults;
    
        arrayResults = hash._slots
        .filter(_slot => _slot !== undefined)
        .map(_slot => _slot.value);
    console.log(arrayResults);
    
}



// function anagram(stringA, stringB){
//     if(a ===b ){
//         return true;
//     }
//     if(!stringA || !stringB){
//         return false;
//     }
//     if(stringA.length !== stringB.length){
//         return false;
//     }
//     let arrayA = stringA.toLowerCase().toCharArray();
//     let arrayB = stringB.toLowerCase().toCharArray();
//     let occurrences = [];
// //sort into differnet groups by hash value
// //if string has value of each letter
// //need an array to hold words that are anagrams
//     for(let i =0; i < string.length; i++){

//     }

// }

HashMap.MAX_LOAD_RATIO = 0.9;
HashMap.SIZE_RATIO = 3;

function main(){
    const lor = new HashMap();
    lor.set("Hobbit", "Bilbo");
    lor.set("Hobbit", "Frodo");
    // lor.set("Wizard","Gandolf");
    // lor.set("Human","Aragon");
    // lor.set("Elf", "Legolas");
    // lor.set("Maiar","The Necromancer");
    // lor.set("Maiar","Sauron");
    // lor.set("RingBearer", "Gollum");
    // lor.set("LadyOfLight", "Galadriel");
    // lor.set("HalfElven", "Arwen");
    // lor.set("Ent", "Treebeard");
    // console.log(is_palindrome('racecar'));
    // console.log(is_palindrome('acecarr'));
    // console.log(is_palindrome('mom'));
    // console.log(lor.get("Maiar"));
    console.log(anagram(['east', 'cars', 'acre', 'arcs', 'teas', 'eats', 'race']));
    //console.log(lor);
}
main();
